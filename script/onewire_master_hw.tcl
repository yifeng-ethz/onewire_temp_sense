...
add_fileset_file crc_checker_maxim_crc8.vhd VHDL PATH ../rtl/vhdl/onewire_master/crc_checker_maxim_crc8.vhd
add_fileset_file pseudo_clock_down_convertor.vhd VHDL PATH ../rtl/common/pseudo_clock_down_convertor.vhd
add_fileset_file onewire_master.vhd VHDL PATH ../rtl/vhdl/onewire_master/onewire_master.vhd TOP_LEVEL_FILE
...
add_parameter USE_IRQ boolean true 
set_parameter_property USE_IRQ HDL_PARAMETER false
set_parameter_property USE_IRQ DISPLAY_NAME "Enable IRQ interface"
set dscpt \
"<html>
Enable IRQ interface, which will inform the processor. <br>
You may disable it if using the dedicated controller. <br>
IRQ is asserted once the command is completed or terminated with error. <br>
Once Asserted, any read or write to the memory-mapped interface <i>ctrl</i> will unset the irq. <br>
<br>
<b>Note:</b> As a pure indicator, IRQ has no functional effect this IP, so the processor may safely ignore it even it is asserted. 
</html>"
set_parameter_property USE_IRQ LONG_DESCRIPTION $dscpt
set_parameter_property USE_IRQ DESCRIPTION $dscpt
...
add_display_item "Ports" USE_IRQ PARAMETER
## ---- 
...
proc my_elaborate {} {
	...
	# ----
	# close irq interface complete
	set enable_irq [get_parameter_value USE_IRQ]
	set_port_property ins_complete_irq termination [expr {!$enable_irq}]
